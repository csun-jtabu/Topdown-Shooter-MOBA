...
# this is the python version of the level generation script
...
def markFlippedAndOrRotatedGridPieces(intermediary_matrix, result_array_2d, friendlyTowerCoordinates,
                                      friendlyStartCoordinates):
...
def main():
    ...
    for currentCoordinatePosition in range(len(returnedSetOfCoordinatesCopy)):
        ...
        squareRootResult, potentialFarthestCoordinate = findEuclidianDistance(centerCoordinate,
                                                                              returnedSetOfCoordinatesCopy,
    ...
    if (RANDOM_MIRROR_CHOICE == 0 or RANDOM_MIRROR_CHOICE == 1):
        ...
        returnedFriendlyResults = markFlippedAndOrRotatedGridPieces(intermediary_matrix, result_array_2d,
                                                                    friendlyTowerCoordinates, friendlyStartCoordinates)
    ...
    for currentRowPosition in range(len(result_array_2d)):
        for currentColumnPosition in range(len(result_array_2d)):
            ...
            if ((coordinate_only_array_2d[currentRowPosition][currentColumnPosition] == 1) or (
                    flipped_coordinate_only_array_2d[currentRowPosition][currentColumnPosition] == 1)):
                set_box(result_array_2d, currentRowPosition, currentColumnPosition, INTERMEDIARY_BOX_SIZE)

            # Generate tower rooms.
            if ((coordinate_only_array_2d[currentRowPosition][currentColumnPosition] == 2) or (
                    flipped_coordinate_only_array_2d[currentRowPosition][currentColumnPosition] == 2)):
                set_box(result_array_2d, currentRowPosition, currentColumnPosition, TOWER_BOX_SIZE)

            # Generate start rooms.
            if ((coordinate_only_array_2d[currentRowPosition][currentColumnPosition] == 4) or (
                    flipped_coordinate_only_array_2d[currentRowPosition][currentColumnPosition] == 4)):
    ...
    for currentRowPosition in range(numpy_result_array_2d.shape[0]):
        for currentColumnPosition in range(numpy_result_array_2d.shape[1]):
            ...
            if (flipped_coordinate_only_array_2d[currentRowPosition][currentColumnPosition] == 1):
                ...
                if (coordinate_only_array_2d[currentRowPosition][currentColumnPosition] ==
                        flipped_coordinate_only_array_2d[currentRowPosition][currentColumnPosition]):
    ...
    #
    # def rerunNotebook():
    #     pm.execute_notebook('LevelPatternGeneration.ipynb', None)
    ...
    if (enemyTowerCoordinates == []):
        ...
        distanceBetweenFriendlyAndEnemyTowers = \
        findEuclidianDistance(friendlyTowerCoordinates, enemyTowerCoordinates, 0)[0]
    ...
    if ((GENERATE_SPAWN_BOX == 1) and (enemyStartCoordinates == [])):
        ...
        if (GENERATE_SPAWN_BOX == 1):
            ...
            distanceBetweenFriendlyAndEnemyStarts = \
            findEuclidianDistance(friendlyStartCoordinates, enemyStartCoordinates, 0)[0]
    ...
    if (safeToDrawImage == True):
        ...
        resizedImage = cv2.resize(bgr_image, (64, 64))
        cv2.imwrite('output.png', resizedImage)
pass
