...
while (rerunEverythingCheck):
    ...
    def generate_coordinates(minCoordinates, maxCoordinates, numberOfCoordinates):
        ...
        while (len(setOfCoordinates) < numberOfCoordinates):
            ...
            if (random_side(RANDOM_MIRROR_CHOICE, randomXCoordiante, randomYCoordinate, MAX_RANGE)):
                ...
                #print((randomXCoordiante, randomYCoordinate))
                #print()
    ...
    '''
    # Get flipped tower coordinates to mark end of path.
    get_flipped_tower_and_start = [[0 for _ in range(cols)] for _ in range(rows)]
    get_flipped_tower_and_start[towerCenterRow][towerCenterCol] = 1
    get_flipped_tower_and_start[startCenterRow][startCenterCol] = 2

    if (RANDOM_MIRROR_CHOICE == 0 or RANDOM_MIRROR_CHOICE == 1):
        # leftHalfCheck and rightHalfCheck
        intermediary_matrix = np.fliplr(get_flipped_tower_and_start)
        intermediary_matrix = intermediary_matrix.tolist()
    elif (RANDOM_MIRROR_CHOICE == 2 or RANDOM_MIRROR_CHOICE == 3):
        # topHalfCheck and bottomHalfCheck
        intermediary_matrix = np.flipud(get_flipped_tower_and_start)
        intermediary_matrix = intermediary_matrix.tolist()
    ...
    for currentPosition in range(lengthOfReturnedSetOfCoordinates):
        if (currentPosition > 0):
            ...
            try:
                ...
                #print("Returned A* Path: ")
                #print(returnedAStarPath)
                #print()
    ...
    if (safeToDrawImage == True):
        # Write coordinates to txt file (and overwrite anything currently there).
        coordinateFile = open("coordinate_file.txt", "w")
        coordinateFile.write("All Friendly Coordinates: ")

        currentPositionCountA = 0
        currentPositionFoundCheck = False
        currentCoordinateLine = ""

        foundTowerCenterRow = 0
        foundTowerCenterCol = 0

        for i in range(len(coordinate_only_array_2d)):
            for j in range(len(coordinate_only_array_2d[i])):
                ...
                #
                #if (currentPositionFoundCheck):
                #    currentPositionCountA += 1

                if (coordinate_only_array_2d[i][j] == 2):
                    foundTowerCenterRow = i
                    foundTowerCenterCol = j

        coordinateFile.write(currentCoordinateLine)
        ...
        coordinateFile.write(f"({foundTowerCenterRow},{foundTowerCenterCol})")
        print("Friendly Tower Coordinates: " + f"({foundTowerCenterRow},{foundTowerCenterCol})")


        coordinateFile.write("\n\nAll Enemy Coordinates: ")
        currentCoordinateLine = ""
        currentPositionCountB = 0
        #savedI = i
        #savedJ = j
        savedI = 0
        savedJ = 0

        for i in range(len(flipped_coordinate_only_array_2d)):
            for j in range(len(flipped_coordinate_only_array_2d[i])):
                ...
                if (flipped_coordinate_only_array_2d[i][j] == 2):
                    savedI = i
                    savedJ = j
